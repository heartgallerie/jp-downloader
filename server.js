...
const MAX_QUEUE = Math.max(0, Number(process.env.MAX_QUEUE || 10));
...
function mediaHeaders(watch){const u=new URL(watch);return{accept:'*/*',referer:watch,origin:u.origin,'user-agent':BROWSER_UA}}
function ffmpegHeaders(headers){return Object.entries(headers||{}).map(([k,v])=>`${k}: ${v}\r\n`).join('')}
async function directSources(watch,html,job){
  ...
  const headers={accept:'application/json, text/plain, */*',referer:watch,'user-agent':BROWSER_UA};
  for(const id of ids){
    ...
    try{
      ...
      if(j&&Array.isArray(j.sources)){
        job.apiUrl=api;
        job.mediaHeaders=mediaHeaders(watch);
...
async function resolve(watch,job){
  ...
  job.browser=browser;
  try{
    const page=await browser.newPage({userAgent:BROWSER_UA,viewport:{width:1280,height:720},locale:'en-US'});
    let found=null;
    const capture=async r=>{if(found||!/\/api\/v1\/episodes\/[^/]+\/sources(?:\?|$)/i.test(r.url()))return;try{const j=await r.json();if(j&&Array.isArray(j.sources)){found={apiUrl:r.url(),json:j};job.mediaHeaders=mediaHeaders(watch)}}catch{}};
    ...
    const sourceUrl=mediaValid(hls.file);job.apiUrl=found.apiUrl;job.hlsUrl=sourceUrl.href;job.mediaHeaders=job.mediaHeaders||mediaHeaders(watch);job.status='Found HLS source.';return sourceUrl.href;
  }finally{await browser.close().catch(()=>{});job.browser=null}
}
async function playlist(hls,dir,headers){const source=mediaValid(hls),base=source.href.endsWith('/')?source.href:source.href+'/';const requestHeaders=headers||mediaHeaders(base);const indexUrl=new URL('index.json',base);let r=await fetchWithTimeout(indexUrl,{headers:requestHeaders});if(r.status===403){console.warn(`[playlist] index.json returned 403; retrying with source-host referer.`);const retryHeaders={...requestHeaders,referer:source.origin+'/',origin:source.origin};r=await fetchWithTimeout(indexUrl,{headers:retryHeaders})}if(!r.ok)throw Error(`Could not fetch the video playlist (${r.status}).`);const lines=(await r.text()).split(/\r?\n/);const out=lines.map(line=>{const t=line.trim();if(!t||t.startsWith('#'))return line;try{return new URL(t,base).href}catch{return t}});for(const line of out){const t=line.trim();if(t&&!t.startsWith('#'))mediaValid(t)}const file=path.join(dir,'local.m3u8');await fsp.writeFile(file,out.join('\n'),'utf8');return file}
async function duration(file,job){try{const args=['-v','error','-show_entries','format=duration','-of','default=nw=1:nk=1','-protocol_whitelist','file,http,https,tcp,tls,crypto,data','-allowed_extensions','ALL','-extension_picky','0'];if(job?.mediaHeaders)args.push('-headers',ffmpegHeaders(job.mediaHeaders));args.push(file);let out='';await proc(FFPROBE,args,x=>out+=x,job);const n=parseFloat(out.trim());return Number.isFinite(n)&&n>0?n:0}catch{return 0}}
function queuePosition(job){const i=queue.indexOf(job.id);return i===-1?0:i+1}
function pumpQueue(){while(running.size<MAX_CONCURRENT&&queue.length){const id=queue.shift(),job=jobs.get(id);if(!job||job.cancelled||job.done)continue;running.add(id);job.running=true;job.status='Starting…';convert(job,job.watch).finally(()=>{running.delete(id);job.running=false;pumpQueue()})}}
async function convert(job,watch){let dir;try{const u=new URL(watch),parts=u.pathname.split('/').filter(Boolean),slug=parts.pop()||'video',ep=u.searchParams.get('ep');dir=await fsp.mkdtemp(path.join(os.tmpdir(),'hls-converter-'));job.dir=dir;const hls=await resolve(watch,job);if(job.cancelled)throw Error('Cancelled');job.status='Downloading playlist…';const m3u8=await playlist(hls,dir,job.mediaHeaders);job.duration=await duration(m3u8,job);if(job.cancelled)throw Error('Cancelled');const sourceName=job.displayName||slug;const name=safeName(ep?`${sourceName}-ep-${ep}`:sourceName);job.name=ep?`${sourceName} · Episode ${ep}`:sourceName;job.filename=`${name}.mp4`;const out=path.join(dir,`${name}.mp4`);job.status='Converting with fast encoding (~2.5× faster)…';const ffargs=['-protocol_whitelist','file,http,https,tcp,tls,crypto,data','-allowed_extensions','ALL','-extension_picky','0'];if(job.mediaHeaders)ffargs.push('-headers',ffmpegHeaders(job.mediaHeaders));ffargs.push('-i',m3u8,'-c:v','libx264','-preset','ultrafast','-crf',process.env.FFMPEG_CRF||'20','-c:a','aac','-b:a','192k','-movflags','+faststart',out);await proc(FFMPEG,ffargs,line=>{const m=line.match(/time=(\d+):(\d+):(\d+(?:\.\d+)?)/);if(m){job.seconds=+m[1]*3600+ +m[2]*60+ +m[3];if(job.duration)job.progress=Math.min(99,job.seconds/job.duration*100)}},job);if(job.cancelled)throw Error('Cancelled');job.status='Verifying MP4…';const probeArgs=['-v','error','-show_entries','format=format_name','-of','default=nw=1'];if(job.mediaHeaders)probeArgs.push('-headers',ffmpegHeaders(job.mediaHeaders));probeArgs.push(out);await proc(FFPROBE,probeArgs,null,job);if(job.cancelled)throw Error('Cancelled');job.output=out;job.progress=100;job.status='Done';job.done=true}catch(e){if(job.cancelled||e.message==='Cancelled'){job.status='Cancelled';job.error=null}else{console.error(`[job ${job.id}]`,e);job.status='Error';job.error='The conversion failed. Please try again.'}job.done=true}finally{job.process=null;job.browser=null}}
app.post('/api/convert',(req,res)=>{try{if(rateLimited(req))return res.status(429).json({error:'Too many conversion requests. Please wait a few minutes and try again.'});if(running.size>=MAX_CONCURRENT&&queue.length>=MAX_QUEUE)return res.status(429).json({error:'The conversion queue is full right now. Please try again shortly.'});const u=valid(req.body?.url),id=crypto.randomUUID(),job={id,method:'fast',status:'Queued…',done:false,createdAt:Date.now(),progress:0,seconds:0,duration:0,cancelled:false,watch:u.href,running:false};jobs.set(id,job);queue.push(id);pumpQueue();res.json({id})}catch(e){res.status(400).json({error:e.message})}});
app.get('/api/jobs/:id',(req,res)=>{const j=jobs.get(req.params.id);if(!j)return res.status(404).json({error:'Job not found.'});const pos=j.running||j.done?0:queuePosition(j);res.json({id:j.id,status:j.status,done:!!j.done,error:j.error||null,seconds:j.seconds||0,duration:j.duration||0,progress:j.progress||0,name:j.name||null,filename:j.filename||null,method:'fast',queuePosition:pos,queueLength:queue.length,download:j.done&&j.output?`/api/jobs/${j.id}/download`:null,stream:j.done&&j.output?`/api/jobs/${j.id}/stream`:null})});
app.delete('/api/jobs/:id',async(req,res)=>{const j=jobs.get(req.params.id);if(!j)return res.status(404).json({error:'Job not found.'});j.cancelled=true;const qi=queue.indexOf(j.id);if(qi!==-1)queue.splice(qi,1);j.status='Cancelling…';if(j.running){try{if(j.process)await killProcessTree(j.process)}catch{}try{if(j.browser)await j.browser.close()}catch{}}if(j.dir){try{await fsp.rm(j.dir,{recursive:true,force:true})}catch{}}j.process=null;j.browser=null;j.done=true;j.output=null;j.status='Cancelled';j.error=null;return res.json({ok:true,status:'Cancelled'})});
...
setInterval(async()=>{const cut=Date.now()-7200000;for(const[id,j]of jobs)if(j.createdAt<cut){try{if(j.process)await killProcessTree(j.process)}catch{}try{if(j.browser)await j.browser.close()}catch{}try{await fsp.rm(j.dir,{recursive:true,force:true})}catch{}const qi=queue.indexOf(id);if(qi!==-1)queue.splice(qi,1);running.delete(id);jobs.delete(id)}for(const[key,times]of rate){if(!times.some(t=>Date.now()-t<RATE_WINDOW_MS))rate.delete(key)}pumpQueue()},600000).unref();
