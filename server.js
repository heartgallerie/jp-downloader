...
async function resolve(watch,job){
  ...
  job.browser=browser;
  try{
    const page=await browser.newPage({userAgent:'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/140 Safari/537.36',viewport:{width:1280,height:720},locale:'en-US'});
    let found=null;
    const capture=async r=>{if(found||!/\/api\/v1\/episodes\/[^/]+\/sources(?:\?|$)/i.test(r.url()))return;try{const j=await r.json();if(j&&Array.isArray(j.sources))found={apiUrl:r.url(),json:j}}catch{}};
    ...
    const sourceUrl=mediaValid(hls.file);job.apiUrl=found.apiUrl;job.hlsUrl=sourceUrl.href;job.status='Found HLS source.';return sourceUrl.href;
