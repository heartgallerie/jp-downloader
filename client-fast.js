...
let jobId=null,objectUrl=null,cancelled=false,controller=null,ffmpeg=null;
...
async function run(e){
  ...
  }finally{convert.disabled=false;cancel.disabled=true;controller=null;if(cancelled&&ffmpeg){try{await ffmpeg.terminate()}catch{}ffmpeg=null}}
}
form.onsubmit=run;
cancel.onclick=e=>{e.preventDefault();e.stopPropagation();if(convert.disabled){cancelled=true;cancel.disabled=true;cancel.textContent='Cancelling…';if(controller)controller.abort();if(jobId)fetch(`/api/jobs/${jobId}`,{method:'DELETE'}).catch(()=>{})}};
